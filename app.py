...
def model_options() -> dict[str, Path | str]:
    return {
        "faster-whisper medium 本地模型": MODEL_MEDIUM,
        "faster-whisper base 本地模型": MODEL_BASE,
        "base 在线/缓存模型名": "base",
        "medium 在线/缓存模型名": "medium",
    }


def model_available(model_value: Path | str) -> bool:
    if isinstance(model_value, Path):
        required = ["config.json", "model.bin", "tokenizer.json", "vocabulary.txt"]
        return model_value.exists() and all((model_value / name).exists() for name in required)
    return True


def show_model_status(label: str, model_value: Path | str) -> None:
    if isinstance(model_value, Path):
        ok = model_available(model_value)
        box_class = "status-ok" if ok else "status-warn"
        status = "已找到本地模型" if ok else "本地模型文件不完整"
        st.markdown(
            f"""
            <div class="status-box {box_class}">
              <strong>{status}</strong><br>
              <span class="small-muted">{label}: {model_value}</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div class="status-box status-warn">
              <strong>将使用模型名加载</strong><br>
              <span class="small-muted">如果本地没有缓存，程序会尝试联网下载：{model_value}</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def show_metrics(output_dir: Path) -> None:
    ...
    cols[1].metric("Display CER", format_metric(metrics.get("display_cer")))
    cols[2].metric("运行时间", f"{metrics.get('runtime_seconds', 0):.1f}s")
    cols[3].metric("原始片段", metrics.get("raw_segment_count", "n/a"))
    cols[4].metric("展示片段", metrics.get("segment_count", "n/a"))
...
st.title("会议音频智能助手")
st.caption("默认使用本地 faster-whisper-medium 模型，不使用纠错表，适合展示真实 ASR 准确率。")
...
with st.sidebar:
    ...
    compute_type = st.selectbox("计算精度", ["int8", "float32"], index=0, disabled=mode != "真实 ASR")
    cpu_threads = st.slider("CPU 线程", min_value=1, max_value=4, value=1, disabled=mode != "真实 ASR")
...
with main_tab:
    ...
    with left:
        transcript_text = st.text_area(
            "已有 transcript（仅在“使用已有 transcript”模式生效）",
    ...
    if run_clicked:
        if audio_path is None:
            st.error("还没有选择音频文件。")
        elif mode == "真实 ASR" and not model_available(selected_model):
            st.error("所选本地模型不完整，请先确认 models 目录中有 config.json、model.bin、tokenizer.json、vocabulary.txt。")
        else:
            ...
            engine = "demo" if mode == "使用已有 transcript" else "faster-whisper"
            model = str(selected_model) if mode == "真实 ASR" else "tiny"
            ...
            with st.spinner("正在运行处理流程。medium 模型在 CPU 上通常需要几分钟，请稍等..."):
                try:
