...
本仓库不上传课程演示音频，避免把大文件或不确定版权的数据放进 GitHub。运行时可以：

- 在 GUI 中选择“上传自定义音频”；
- 或者在命令行中把 `path\to\audio.wav` 和 `path\to\transcript.txt` 换成你本机的真实文件路径。
...
python run_pipeline.py `
  ...
  --transcript-file path\to\transcript.txt `
  --reference-file path\to\transcript.txt `
  --output outputs\real_dialogue_human_transcript `
  --engine demo `
  --language zh `
  --speakers 5 `
  --diarizer cluster
...
python run_pipeline.py `
  --audio path\to\audio.wav `
  --reference-file path\to\transcript.txt `
  --output outputs\real_asr_base_cluster `
...
python check_asr_setup.py
python test_whisper_load.py
```
...
python train_audio_classifier.py `
  --data-dir data\labeled_audio `
  --output outputs\audio_classifier `
  --test-size 0.3
```
